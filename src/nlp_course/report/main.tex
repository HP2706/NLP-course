...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item We note that there are more answerable than unanswerable by a factor of 10-1.Which means it would be uninformative rely on metrics like accuracy.
    ...
    To get a meaningful representation of the most informative words, we filter out stopwords using spaCy's language-specific stopword lists. We also remove common symbols and tokens not included in the stopword lists.
    For Finnish and Russian, which use spaces to separate words, we extract distinct words by splitting on spaces. For Japanese, which doesn't use spaces in the same way, we employ a language-specific tokenizer.
    ...
    We plot for each language the question word length against the answerability of the question.
    ...
    Performance is poor, and this is expected as there are many downsides to this approach: 
    ...
    Furthermore we are putting a lot of faith in the tokenization step. And the predictions of our model could meaningfully change with a different tokenization scheme.
...
Overall we find that the inclusion of context in most cases hearts performance. for instance both finnish and japanese classifiers perform better without context and this seems to be the case for most tasks. This is quite puzzling.  
...
Suprinsingly for the finnish and japanese classifiers perform better on the russian validation set than the validation set they themselves were trained on and moreover they perform better than classifiers trained on the respective training set. 
For instance the Japanese classifier beats the Russian classifier.
...
DistilBERT as a baseline and DeBERTa-V3 as a state-of-the-art model. DeBERTa performs the best.
However, we note that there is a significant discrepancy between how the model performs on Finnish versus Russian and Japanese.
This is expected as DistilBERT and DeBERTa are not explicitly trained on the respective languages.
Thus, we also examine using the mT5-base model, which is a T5 encoder-decoder model trained on all 3 target languages (https://huggingface.co/google/mt5-base).
Instead of performing a full fine-tuning, we freeze the weights and use a LoRA adapter \cite{hu2021loralowrankadaptationlarge}.
...
We train using the chatml template, to ensure no bugs or inconsistencies between inference and training are introduced. 
The prompting setup can be found in the Appendix~\vref{sec:prompting}.
...
we measure performance on the validation set before and after finetuning and  
observe that although the performance improved, the end result is still somewhat disappointing.
